...
BUILD_IDENTITY_SCRIPT="$ROOT_DIR/scripts/lib/build-identity.sh"
ANDROID_VERSION_SCRIPT="$ROOT_DIR/scripts/android-version.sh"
IOS_VERSION_SCRIPT="$ROOT_DIR/scripts/ios-version.sh"
LOG_ROOT="${QUOTADOG_RELEASE_LOG_ROOT:-$ROOT_DIR/build/release/test-tracks}"
LOCK_DIR="${QUOTADOG_RELEASE_LOCK_DIR:-$ROOT_DIR/build/release/.publish-tracks.lock}"
ANDROID_TRACK="${QUOTADOG_ANDROID_TRACK:-internal}"
MAKE_COMMAND="${QUOTADOG_MAKE_COMMAND:-}"
...
usage() {
  cat <<'EOF'
Prepare and publish QuotaDog to the Google Play internal track and TestFlight.
...
if [[ -n "$MAKE_COMMAND" ]]; then
  case "$MAKE_COMMAND" in
    /*) ;;
    *) die "QUOTADOG_MAKE_COMMAND must be an absolute executable path." ;;
  esac
  [[ -x "$MAKE_COMMAND" ]] ||
    die "QUOTADOG_MAKE_COMMAND is not executable: $MAKE_COMMAND"
else
  MAKE_COMMAND="$(command -v make || true)"
  [[ -n "$MAKE_COMMAND" ]] || die "make is required."
fi

if [[ -n "${QUOTADOG_RELEASE_LOCK_DIR:-}" ]]; then
  [[ -n "${QUOTADOG_MAKE_COMMAND:-}" ]] ||
    die "QUOTADOG_RELEASE_LOCK_DIR is only available with an injected test Make command."
  case "$LOCK_DIR" in
    /*) ;;
    *) die "QUOTADOG_RELEASE_LOCK_DIR must be an absolute path." ;;
  esac
fi
...
PRODUCT_VERSION="$("$PRODUCT_VERSION_SCRIPT" show "$ROOT_DIR")" ||
  die "Could not read the product version."
ANDROID_VERSION="$("$ANDROID_VERSION_SCRIPT" show)" ||
  die "Could not read the Android version."
IOS_VERSION="$("$IOS_VERSION_SCRIPT" show)" ||
  die "Could not read the iOS version."
SOURCE_REVISION="$("$BUILD_IDENTITY_SCRIPT" field short "$ROOT_DIR")" ||
  die "Could not read the source revision."
...
# The width is queried once and refreshed only on SIGWINCH, so steady-state
# frames spawn no subprocess for it.
TERMINAL_WIDTH=""
...
# Frame assembly for the flicker-free renderer: every line ends with
# erase-to-end-of-line, so the previous frame stays visible until each row is
# atomically overwritten. A full-screen erase would flash blank between the
# erase and the repaint.
TUI_FRAME=""
frame_line() {
  TUI_FRAME+="$1"$'\033[K\n'
}
...
render_tui() {
  ...
  divider="${divider// /-}"
  TUI_FRAME=""
  frame_line "QuotaDog test-track release  $PRODUCT_VERSION  ·  $SOURCE_REVISION"
  frame_line "$divider"
  frame_line "Android / Google Play internal  $ANDROID_VERSION  [$ANDROID_STATUS]"
  append_recent_log "$ANDROID_LOG" "$width"
  frame_line ""
  frame_line "iOS / TestFlight                $IOS_VERSION  [$IOS_STATUS]"
  append_recent_log "$IOS_LOG" "$width"
  frame_line ""
  frame_line "Overall: $OVERALL_STATUS"
  frame_line "Logs: $LOG_DIR"
  # One write per frame: home the cursor, overwrite in place, erase whatever
  # remains below a shrinking frame, and wrap it in a synchronized update so
  # capable terminals repaint exactly once.
