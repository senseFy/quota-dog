...
static QDPalette QDLightPalette(void) {
    return (QDPalette){
        ...
        .devinAccent = QDHex(0x4C4A6E),
        .droidAccent = QDHex(0xC76B2D),
    };
}
...
static QDPalette QDDarkPalette(void) {
    return (QDPalette){
        ...
        .devinAccent = QDHex(0xB0AFD6),
        .droidAccent = QDHex(0xE29A6B),
    };
}
...
static NSColor *QDProviderAccent(NSString *provider, QDPalette palette) {
    ...
    if ([provider isEqualToString:@"DEVIN"]) return palette.devinAccent;
    if ([provider isEqualToString:@"DROID"]) return palette.droidAccent;
...
// Devin mark from lobehub icons-static-svg 1.95.0 (icons/devin.svg).
static NSString * const kQDPathDevin =
...
static CGFloat QDProviderIconScale(NSString *provider) {
    ...
    if ([provider isEqualToString:@"DEVIN"]) return 0.74;
    if ([provider isEqualToString:@"DROID"]) return 0.78;
    return 0.70; // CODEX
}
...
static NSString *QDProviderPathData(NSString *provider) {
    ...
    if ([provider isEqualToString:@"DEVIN"]) return kQDPathDevin;
    if ([provider isEqualToString:@"DROID"]) return kQDPathDroid;
    return kQDPathCodex;
}
...
static NSImage *QDProviderLogoImage(NSString *provider, NSColor *tint, CGFloat pointSize) {
    ...
    NSInteger viewBox = [provider isEqualToString:@"DROID"] ? 508 : 24;
    NSString *svg = [NSString stringWithFormat:
        @"<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
        @"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"%ld\" height=\"%ld\" viewBox=\"0 0 %ld %ld\">"
        @"<path fill=\"#000000\" fill-rule=\"evenodd\" d=\"%@\"/>"
        @"</svg>",
        (long)px, (long)px, (long)viewBox, (long)viewBox, pathData];
